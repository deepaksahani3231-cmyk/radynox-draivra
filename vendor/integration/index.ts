...
export default ({ config: _themeConfig = 'src/config.yaml' } = {}): AstroIntegration => {
  let cfg: AstroConfig;
  return {
    name: 'radynox-draivra-integration',
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('radynox-draivra');

        const virtualModuleId = 'radynox-draivra:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-radynox-draivra-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`Radynox-Draivra \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`Radynox-Draivra config has been loaded.`);
        }
      },
